...
@main.command()
@click.option('--dataset', '-d', default='medical_texts', 
...
def run(dataset: str, csv_file: Optional[str], jsonl_file: Optional[str], 
        output: Optional[str]):
...
def lf_list():
    ...
    for name, lf in LF_REGISTRY.items():
        ...
        if lf.n_calls > 0:
            click.echo(f"    Coverage: {lf.coverage:.2%}, Error rate: {lf.error_rate:.2%}")


@main.command()
@click.option('--dataset', '-d', default='medical_texts',
              help='Dataset to test on')
@click.option('--sample-size', '-n', default=10, type=int,
...
'''
    ...
    click.echo("Edit the file to implement your labeling logic, then import it to register the LF.")
...
def lf_stats(output_format: str):
    ...
    if output_format == 'json':
        ...
        click.echo(f"{'Name':<20} {'Coverage':<10} {'Error Rate':<12} {'Calls':<8} {'Tags'}")
        click.echo("-" * 70)
